...
for e in range(nb_epochs):
    ...
    for dset in Data_Set_Train:    
        for cell in dset[0]:
            ...
            if os.path.isfile(cell+'_XY.npz'):
                try:
                    data = np.load(cell+'_XY.npz')
                    X_train = data['X']
                    Y_train = data['Y']
                    print("npz is OK")
                    history1 = model.fit_generator(datagen.flow(X_train, Y_train, batch_size=batch_size), steps_per_epoch=len(X_train) / batch_size, epochs=1, callbacks = [early,checkpoint])
                except: 
                    print("npz is bad")
                    pass
model.save('inceptionV3_SLAV_3_1.h5')
        ...
        for cell in dset[0]:
            ...
            if os.path.isfile(cell+'_XY.npz'):
                try:
                    ...
                    Y_train = data['Y']
                    print("npz is OK")
                    history2 = model.fit_generator(datagen.flow(X_train, Y_train, batch_size=32), steps_per_epoch=len(X_train) / 32, epochs=1, callbacks = [early,checkpoint])
                except: 
                    print("npz is bad")
                    pass
model.save('inceptionV3_SLAV_3_2.h5')
